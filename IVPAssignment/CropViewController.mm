...
-(void)buttonsScroll{
    ...
    NSArray *butArr=@[@"",@"noedit_filled",@"original_filled",@"blackwhite_filled",@"magic_filled",@"Gallery",@"default_filled"];
    NSArray *labArr=@[@"",@"Original",@"Gray Scale",@"Black & White",@"Gaussian Blur",@"Erosion & Dilation",@"Default"];
...
-(void)scrolllButAction:(UIButton *)sender{
    switch (sender.tag) {
        ...
        case 3:
            ...
            break;
        case 4:
            _sourceImageView.image= [self gaussianBlurFilter:_cropImage];
            break;
        case 5:
            _sourceImageView.image= [self erosionandDilation:_cropImage];
            break;
        case 6:
...
void find_squares(cv::Mat& image, std::vector<std::vector<cv::Point>>&squares) {
    ...
    GaussianBlur(image, blurred, cvSize(11,11), 0);// gaussian blur for more accuracy of square detection
...
-(UIImage *)gaussianBlurFilter:(UIImage *)processedImage{
    cv::Mat grayImage = [MMOpenCVHelper cvMatGrayFromAdjustedUIImage:processedImage];
    cv::Mat blurred(grayImage);
    cv::GaussianBlur(grayImage, blurred, cvSize(11,11), 0);
    
    UIImage *gaussianImage=[MMOpenCVHelper UIImageFromCVMat:grayImage];
    grayImage.release();
...
-(UIImage *) erosionandDilation:(UIImage *)processedImage
{
    cv::Mat src = [MMOpenCVHelper cvMatGrayFromAdjustedUIImage:processedImage];
...
//    int const max_kernel_size = 21;
    ...
    cv::Mat element = cv::getStructuringElement( erosion_type,
                                                ...
                                                cvPoint( erosion_size, erosion_size ) );
    /// Apply the erosion operation
    erode( src, new_image, element );
    src = new_image;
    int dilation_type = 0;
    ...
    cv::Mat element1 = cv::getStructuringElement( dilation_type,
                                                 cvSize( 2*dilation_size + 1, 2*dilation_size+1 ),
                                                 cvPoint( dilation_size, dilation_size ) );
    dilate( src, new_image, element );
    
    UIImage *imgED=[MMOpenCVHelper UIImageFromCVMat:new_image];
    new_image.release();
